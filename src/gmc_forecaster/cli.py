...
def _cmd_forecast(a: argparse.Namespace) -> None:
    ...
    mode_note = (
        f", режим {m['mode']}"
        + (
            f" τ={m['gap_tau']}, история разрыва {m.get('gap_hist', 0)} кв."
            if m.get("mode") == "auto"
            else ""
        )
    ...
    if m.get("mode") == "auto" and "вес_якоря" in df:
        # сдвиг уровня заметен там, где вес якоря низкий: разрыв доли признан
        # НЕперсистентным (схлопывается к 1 — рамп/вход), а не устойчивой
        # фирменной гетерогенностью
        flagged = df[df["вес_якоря"].notna() & (df["вес_якоря"] < 0.5)]
        if len(flagged):
            cells = ", ".join(
                f"{r['канал']}{r['продукт']} (разрыв {r['разрыв_доли']}, "
                f"вес {r['вес_якоря']})"
                for _, r in flagged.iterrows()
            )
            print(
                f"ℹ Авто-режим: разрыв доли признан НЕустойчивым, уровень "
                f"сдвинут к модельной доле: {cells}. Разрыв<1 = фирма "
...
def _cmd_backtest(a: argparse.Namespace) -> None:
    ...
    print(
        ...
        f"режим: {s['режим']}"
        + (
            f" (история разрыва до {s['история_разрыва_кв']} кв.)"
            if s["режим"] == "auto"
            else ""
        )
    )
