#!/usr/bin/env python3
"""
gmc_predict.py — прогноз спроса на следующий квартал под сценарием решений.
...
Пример:
  python gmc_predict.py --current W115264.xls \
      --train W1152*.xls W1312*.xls --history Hst*.xlsx \
...
def main() -> None:
    ap = argparse.ArgumentParser(
        description="Прогноз спроса GMC под сценарием решений"
    )
    ap.add_argument(
    ...
    m = df.attrs["meta"]
    print(
        f"Компания {m['company']}, группа {m['group']}: прогноз Q{m['q_now']}->Q{m['q_next']} "
